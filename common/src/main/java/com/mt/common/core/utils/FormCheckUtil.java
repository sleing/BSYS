...
public class FormCheckUtil {
    ...
    public static boolean isFax(String fax) {
        return test(fax, "^(\\d{3,4}-)?\\d{7,8}$");
    }

    /**
     * 是否是邮政编码
     */
    public static boolean isPostal(String postal) {
        return test(postal, "^[1-9]\\d{5}$");
    }

    /**
     * 是否是纳税号
     */
    public static boolean isTax(String tax) {
        return test(tax, "^((\\d{6}[0-9A-Z]{9})|([0-9A-Za-z]{2}\\d{6}[0-9A-Za-z]{10,12}))$");
    }
