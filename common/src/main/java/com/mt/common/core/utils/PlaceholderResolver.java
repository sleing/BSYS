...
public class PlaceholderResolver {
    ...
    public static PlaceholderResolver createResolver(String placeholderPrefix, String placeholderSuffix) {
    ...
    public String resolve(String content, String[] values) {
        Integer start = content.indexOf(this.placeholderPrefix);
        if (start == -1) {
            return content;
        }
        //值索引
        ...
        while (start != -1) {
            Integer end = result.indexOf(this.placeholderSuffix);
    ...
    public String resolveByRule(String content, Function<String, String> rule) {
        ...
        StringBuilder result = new StringBuilder(content);
        while (start != -1) {
            Integer end = result.indexOf(this.placeholderSuffix, start);
            //获取占位符属性值，如${id}, 即获取id
    ...
    public String resolveByMapForSQL(String content, final Map<String, Object> valueMap) {
        return resolveByRule(content, placeholderValue -> {
            if (valueMap.get(placeholderValue) == null) return "NULL";
            Object value = valueMap.get(placeholderValue);
    ...
    public static void main(String[] args) {
        String str = "{" +
                "    \"厂家编号\": \"xxxx\"," +
                "    \"生产厂家\": \"xxx\"," +
                "    \"出厂日期\": \"2021-09-27 9:2:9\"," +
                "    \"出厂批号\": \"xxsxfdsf\"," +
                "    \"代表批量\": \"fdsfdsaf\"," +
                "    \"成型日期\": \"2021-09-28 9:2:17\"," +
                "    \"取样基数\": \"ddd\"," +
                "    \"代号\": \"dddd\"," +
                "    \"品种\": \"普通硅酸盐水泥\"," +
                "    \"强度等级\": \"42.5R\"," +
        ...
        String template = "delete into test (id,name,createDate) values(${id},${name},${createDate},${id})";
        Map<String, Object> valueMap = new HashMap<>();
        valueMap.put("id", 1);
        valueMap.put("name", "xcy");
//        valueMap.put("createDate",new java.util.Date());
        String sql = PlaceholderResolver.getDefaultResolver().resolveByMapForSQL(template, valueMap);
