...
public class FileServerUtil {
    ...
    public static void output(File file, HttpServletResponse response, HttpServletRequest request) {
        int length = (int)file.length();  // 文件总大小
        int start = 0, to = length - 1;  // 开始读取位置, 结束读取位置
        ...
        if (range != null) {
            ...
            if (ranges.length > 1) to = Integer.parseInt(ranges[1].trim());
     ...
     * @param to    结束位置
     */
    public static void output(File file, OutputStream os, Integer size, Integer start, Integer to) {
        ...
        try {
            ...
            if (to == null) {
                ...
                while (to > 0 && (len = is.read(bytes)) != -1) {
                    os.write(bytes, 0, to < len ? (int) ((Integer) to) : len);
                    to -= len;
                }
            }
            os.flush();
