...
import com.mt.common.core.utils.FileServerUtil;
...
public class FileController {
    ...
    public JsonResult upload(@RequestParam MultipartFile file, HttpServletRequest request) {
        try {
            ...
            String uploadUrl=getUploadResult(upload, file.getOriginalFilename(), request).toString();
            System.out.println("-------------------"+getUploadResult(upload, file.getOriginalFilename(), request).toString());
            String[] str=uploadUrl.split(",");
...
//    }
    ...
    public JsonResult resources(@RequestParam MultipartFile file, HttpServletRequest request) {
        try {
            File upload = FileServerUtil.upload(file, getUploadDir(), uploadUuidName);
            String uploadUrl=getUploadResult(upload, file.getOriginalFilename(), request).toString();
            System.out.println(getUploadResult(upload, file.getOriginalFilename(), request).toString());
